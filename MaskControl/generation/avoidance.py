...
os.makedirs(f'{args.path_name}/source', mode=0o755, exist_ok=True)
...
# m_length[k] is a tensor on GPU; convert to a Python int
mlen = int(m_length[k].item())

# Extract (frames, 22, 3) joint positions for sample k
# pred_motions_denorm is already in absolute coordinates
motion_joints = pred_motions_denorm[k, :mlen].detach().cpu().numpy()

# Initialize the converter and save the BVH
bvh_converter = Joint2BVHConvertor()
bvh_filename = f'{args.path_name}/generation.bvh'
bvh_converter.convert(motion_joints, bvh_filename, iterations=10, foot_ik=False)

print(f'BVH file saved to: {bvh_filename}')
